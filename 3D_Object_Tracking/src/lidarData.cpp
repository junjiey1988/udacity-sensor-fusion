...
bool lidarComp(const LidarPoint &lhs, const LidarPoint &rhs)  
{
    return lhs.x < rhs.x;
}
...
void showLidarImgOverlay(cv::Mat &img, std::vector<LidarPoint> &lidarPoints, cv::Mat &P_rect_xx, cv::Mat &R_rect_xx, cv::Mat &RT, cv::Mat *extVisImg)
{
    ...
    auto five_per_it = lidarPoints.begin() + lidarPoints.size() / 20;
    std::nth_element(lidarPoints.begin(), five_per_it, lidarPoints.end(), lidarComp);
    X.at<double>(0, 0) = five_per_it->x;
    X.at<double>(1, 0) = five_per_it->y;
    X.at<double>(2, 0) = five_per_it->z;
    X.at<double>(3, 0) = 1;
